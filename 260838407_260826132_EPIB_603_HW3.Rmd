...
The observed joint effect is a risk ratio of `r cvd.obs.rr` whereas the expected joint effect is `r cvd.exp.rr`. This means there is a sub-multiplicative interacion. The observed joint effect is less than we would expect considereing the observed individual effects. 
...
The observed joint effect is a risk difference of `r cvd.obs.rd` whereas the expected joint effect is `r cvd.exp.rd`. This means there is a super-additive interacion. The observed joint effect is greater than we would expect considereing the observed individual effects. 
...
cvd.exp.rra <- cvd.rr[1,2] + cvd.rr[2,1]
cvd.obs.rra <- cvd.rr[2,2]
...
The observed joint effect is a risk ratio of `r cvd.obs.rra` whereas the expected joint effect of risk ratios on the *additive* scale is `r cvd.exp.rra`. This means there is a sub-additive interacion of the risk ratios, whereas in Q3, we saw that there was a super-additive interaction of te risk *differences*. 
...
a) The interaction contrast is `r ic`. This means that there is some interaction effect because the change in risk for being obese vs not obese is different for those who have high total serum cholesterol compared to those who do not high total serum cholesterol. The IC is positive, which means that there is a super-additive/synergistic effect between obesiety and high total serum cholesterol. We cannot interpret the magnitude of the IC. 

b) The RERI is `r reri`. Since it is not zero, this tells us that there is an interaction between the exposures. Since it is positive, this tells us that the interaction between obesiety and high total serum cholesterol is super-additive/synergistic. The magnitude of the RERI can only be interpreted if we know the baseline risk.......which luckily we do! The baseline risk of cvd is 4.7% and the risk of cvd is increased by 37% 
...
q6.AF <- round(q6.2by2$massoc$PAFRisk.strata.wald, 3)

q6.ans <- paste(q6.AF[1], " with a 95% CI of ", q6.AF[2], ",", q6.AF[3])
...
The population attributable fraction is `q6.ans`.

c) What does AF represent?

The population attrbutable fraction is the proportion of the incidence in the total population that is attributable to the exposure. In this case, it is the proportion of the total number of low weight births due to the smokin mom exposure. If we were to eliminate the exposure (ie: no more smoking moms), then there would be `r q6.ans` fewer low weight births in the entire population. There is also the attributable fraction in the exposed, but that refers to the proportion of outcomes due to the exposure in the exposed group only. 
...
<div id="body">
**Type your answer here:** 
b) What is the odds ratio of low birth weight among smokers?  

The odds ratio for smokers is `r q7.ans.7b`, which means that the odds of having a low weight birth in smokers is 1.82 times greater than the odds of having low birth weight in non-smokers.

c) estimate the proportion of low birthweights that would have been prevented if no mother had smoked during pregnancy     
